...
class _LoginPageState extends State<LoginPage> {
  ...
  _submit() async {
    ...
    if(isValid) {
      setState(() { // setState es para volver a renderizar
        _isFetching = true;
      });
      bool isOk = await _accountAPI.login(_email, _password); // como login retorna un bool a futuro usamos await
      if(isOk){
     ...
     if(_isFetching) // cuando es true ejecuta positioned
       Positioned.fill(
           child: Container(
             ...
             child: Center(
               child: CupertinoActivityIndicator(radius: 15,),// es el simbolo de cargando
             ),
           ))
    ],
    )),);
